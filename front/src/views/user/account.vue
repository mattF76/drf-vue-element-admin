    ...
    <el-card>
      ...
      <el-form :inline="true" :model="queryForm" class="demo-form-inline" style="text-align:left">
        <el-form-item label="账号名">
          <el-input v-model="queryForm.username" placeholder="账号名"></el-input>
        </el-form-item>
      ...
      <el-table ref="multipleTable" :data="table" tooltip-effect="dark" style="width: 100%" @selection-change="handleTableSelectionChange">
        ...
        <el-table-column label="账号角色">
          <template #default="scope">
              ...
              <el-tag v-for="role in scope.row.userRoles" :key="role.id" style="margin-right: 3px;">{{ role.roleName }}</el-tag>
          </template>
        </el-table-column>
        ...
        <div class="demo-drawer__content">
          <el-form :model="dialogForm" :label-width="dialogForm.labelWidth" :label-position="dialogForm.labelPosition" >
            ...
            <el-form-item label="账号角色" >
              <!-- <el-select v-model="dialogForm.userRoles" placeholder="请选择账号角色" style="width: 100%;">
                <el-option label="admin" value="admin"></el-option>
                <el-option label="user" value="user"></el-option>
              </el-select> -->
              <el-select v-model="dialogForm.userRoles" multiple placeholder="账号角色" style="width: 100%;">
                <el-option v-for="role in roleTable" :key="role.id" :label="role.roleName" :value="role.id"/>
              </el-select>
            </el-form-item>
            <!-- <el-form-item label="账号角色" >
              <el-input v-model="dialogForm.userRoles" placeholder="输入账号角色，如‘admin,user’" style="width: 100%;"></el-input>
            </el-form-item> -->
          </el-form>
...
import { getList, delAccount, addAccount, updateAccount } from '@/api/account'
...
export default {
  ...
  data() {
      return {
        ...
        // 所有角色，用于设置账号新增或编辑抽屉的账号角色输入框
        roleTable: [
          {
            'id': '0',
            'roleName': '超级管理员',
            'note': '角色说明',
            'rolePermissions': [{'id':'0', 'permissionName':'账号查看接口'}],
          }
        ]
      }
    },
  ...
  methods: {
    ...
    handleQueryFormQuery() {
      ...
      getList(that.pagination, that.queryForm).then(function(Response){
    ...
    handleQueryFormReset() {
      const that = this;
      that.queryForm.username = "";
      that.queryForm.realName = "";
    ...
    handleButtonGroupDelete() {
      ...
      that.$confirm('确定删除?', '提示', {
        ...
        // console.log("进入批量删除")
        // 发送删除请求
        delAccount(that.multipleSelection).then(function (response) {
        //console.log(response);
          if(response.code == 20000){
            // 删除成功消息提示
    ...
    handleTableEdit(index, row) {
      ...
      // 该变量中存放选中的账号角色id
      const chosenUserRoleIDs = [];
      for (const obj of row['userRoles']) {
        chosenUserRoleIDs.push(obj.id);
      }
      this.dialogForm.userRoles = chosenUserRoleIDs;
    },
    // 表格中删除按钮
    handleTableDelete(index, row) {
      ...
      that.$confirm('确定删除?', '提示', {
        ...
        // 发送删除请求
        delAccount([row]).then(function (response) {
        //console.log(response);
          if(response.code == 20000){
            // 删除成功消息提示
            that.$message({type: 'success', message: response.msg});
    ...
    handleDialogFormSave(){
      ...
      // 新增情况
      if (that.drawer.title == "新增账号"){
        // console.log("新增");
        addAccount(that.dialogForm).then(function (response) {
          // console.log(response);
          if(response.code == 20000){
                that.$notify.info({
                  title: '提示',
      ...
      // 编辑情况
      if (that.drawer.title == "编辑账号"){
        updateAccount(that.dialogForm).then(function (response) {
          // console.log(response);
          if(response.code == 20000){
                that.$notify.info({
                  title: '提示',
    ...
    // 设置抽屉中账号角色所有可选项
    handleDialogFormRolesQuery() {
      const that = this;

      getRoles({'currentPage': 1, 'pageSize': '100'}, {}).then(function(Response){
        // console.log(Response);
        if (Response.code === 20000) {
          that.roleTable = Response.data.items;
        }
      })
      .catch(function (error) {
        console.log(error);
  ...
  created: function() {
    const that = this;
    that.handleQueryFormQuery();
    that.handleDialogFormRolesQuery();
  }
}
</script>
